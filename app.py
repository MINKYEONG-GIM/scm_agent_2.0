...
def compute_store_rows_for_week(
    ...
    for _, r in w.iterrows():
        ...
        # PLC 높음 = 판매 대비 재고 과다 → 잉여 출고 후보.
        # "회전출고 가능 매장" 정의(요청): 최소보유재고량(MIN_STORE_RETAIN_QTY)보다 많고 PLC가 임계(기본 4주) 이상.
        # 실제 넘길 수량(회전가능잉여)은 retain을 남긴 뒤 PLC 초과분만큼만 집계(기존 로직 유지).
...
def compute_weekly_summary_table(
    norm: pd.DataFrame,
    center_df: pd.DataFrame,
    ...
    """
    주차별 요약(전 주차):
    - 결품위험 매장수: 역할=결품위험 고유 매장 수
    - 회전 출고 가능 매장수: 회전출고가능=True 고유 매장 수 (정의는 compute_store_rows_for_week에 반영)
    - 물류센터 보유량: center_stock 전체 합(주차 무관)
    - 추가 입고량: SKU별 취합의 물류+회전_반영_추가발주 합
    - MOQ: 추가 입고가 필요한 SKU들의 MOQ 최댓값(없으면 0)
    - 리드타임: 추가 입고가 필요한 SKU들의 리드타임 최댓값(없으면 0)
    """
    ...
    for yw in weeks:
        ...
        add_in = int(
            pd.to_numeric(summary.get("물류+회전_반영_추가발주", 0), errors="coerce").fillna(0).sum()
        ) if summary is not None and not summary.empty else 0

        need = summary[summary["물류+회전_반영_추가발주"] > 0].copy() if summary is not None and not summary.empty else pd.DataFrame()
